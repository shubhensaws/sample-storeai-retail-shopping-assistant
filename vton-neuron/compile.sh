...
# Compile Qwen-Image-Edit-2511 for Neuron (trn2)
...
COMPILER_WORKDIR="/mnt/nvme/compiler_workdir"
...
VERSION_MODE="all"
if [[ "$1" == "v1" || "$1" == "v2" || "$1" == "v1_flash" || "$1" == "v2_flash" || "$1" == "v3_cp" || "$1" == "v3_cfg" || "$1" == "v3_tp8" || "$1" == "v3_tp16" || "$1" == "v3_tp32" ]]; then
    VERSION_MODE="$1"
    shift
fi

# Use version-specific output directory to keep all compiled models
if [[ "$VERSION_MODE" == "v3_tp8" ]]; then
    COMPILED_MODELS_DIR="/mnt/nvme/compiled_models_tp8"
elif [[ "$VERSION_MODE" == "v3_tp16" ]]; then
    COMPILED_MODELS_DIR="/mnt/nvme/compiled_models_tp16"
elif [[ "$VERSION_MODE" == "v3_tp32" ]]; then
    COMPILED_MODELS_DIR="/mnt/nvme/compiled_models_tp32"
else
...
if [[ "$VERSION_MODE" == "v3_tp8" ]]; then
    echo "  Compiling V3 TP8 (TP=8, DP=2, CFG Parallel + NKI Flash Attention)..."
    echo "  Using TP=8, world_size=16 (DP=2 for batched CFG)"
    python neuron_qwen_image_edit/compile_transformer_v3_cfg.py \
        --height ${HEIGHT} \
        --width ${WIDTH} \
        --tp_degree 8 \
        ...
        --compiled_models_dir ${COMPILED_MODELS_DIR} \
        --compiler_workdir ${COMPILER_WORKDIR}
    echo "  V3 TP8 Transformer compiled successfully!"

    echo ""
    echo "  Compiling V3 Language Model (TP=4, world_size=16)..."
    python neuron_qwen_image_edit/compile_language_model_v3.py \
        --max_sequence_length ${MAX_SEQ_LEN} \
        --batch_size ${BATCH_SIZE} \
        --world_size 16 \
    ...
    echo ""
    echo "  Compiling V3 Vision Encoder (TP=4, world_size=16, float32)..."
    python neuron_qwen_image_edit/compile_vision_encoder_v3.py \
        --image_size ${IMAGE_SIZE} \
        --world_size 16 \
        --compiled_models_dir ${COMPILED_MODELS_DIR} \
...
if [[ "$VERSION_MODE" == "v3_tp16" ]]; then
    echo "  Compiling V3 TP16 (TP=16, DP=2, CFG Parallel + NKI Flash Attention)..."
    echo "  Using TP=16, world_size=32 (DP=2, 24 heads padded to 32, 2 heads/rank)"
    python neuron_qwen_image_edit/compile_transformer_v3_cfg.py \
        --height ${HEIGHT} \
        --width ${WIDTH} \
        --tp_degree 16 \
        ...
        --compiled_models_dir ${COMPILED_MODELS_DIR} \
        --compiler_workdir ${COMPILER_WORKDIR}
    echo "  V3 TP16 Transformer compiled successfully!"

    echo ""
    echo "  Compiling V3 Language Model (TP=4, world_size=32)..."
    python neuron_qwen_image_edit/compile_language_model_v3.py \
        --max_sequence_length ${MAX_SEQ_LEN} \
        --batch_size ${BATCH_SIZE} \
        --world_size 32 \
    ...
    echo ""
    echo "  Compiling V3 Vision Encoder (TP=4, world_size=32, float32)..."
    python neuron_qwen_image_edit/compile_vision_encoder_v3.py \
        --image_size ${IMAGE_SIZE} \
        --world_size 32 \
        --compiled_models_dir ${COMPILED_MODELS_DIR} \
...
if [[ "$VERSION_MODE" == "v3_tp32" ]]; then
    echo "  Compiling V3 TP32 (TP=32, DP=2, CFG Parallel + NKI Flash Attention)..."
    echo "  Using TP=32, world_size=64 (DP=2, 24 heads padded to 32, all 16 devices)"
    python neuron_qwen_image_edit/compile_transformer_v3_cfg.py \
        --height ${HEIGHT} \
        --width ${WIDTH} \
        --tp_degree 32 \
        ...
        --compiled_models_dir ${COMPILED_MODELS_DIR} \
        --compiler_workdir ${COMPILER_WORKDIR}
    echo "  V3 TP32 Transformer compiled successfully!"

    echo ""
    echo "  Compiling V3 Language Model (TP=4, world_size=64)..."
    python neuron_qwen_image_edit/compile_language_model_v3.py \
        --max_sequence_length ${MAX_SEQ_LEN} \
        --batch_size ${BATCH_SIZE} \
        --world_size 64 \
    ...
    echo ""
    echo "  Compiling V3 Vision Encoder (TP=4, world_size=64, float32)..."
    python neuron_qwen_image_edit/compile_vision_encoder_v3.py \
        --image_size ${IMAGE_SIZE} \
        --world_size 64 \
        --compiled_models_dir ${COMPILED_MODELS_DIR} \
...
if [[ "$VERSION_MODE" != "v3_cp" && "$VERSION_MODE" != "v3_cfg" && "$VERSION_MODE" != "v3_tp8" && "$VERSION_MODE" != "v3_tp16" && "$VERSION_MODE" != "v3_tp32" ]]; then
...
# --- Upload compiled artifacts to the TF-managed cache bucket (Phase 4 / D-040) ---
# Set CACHE_S3 (e.g. s3://storeai-<env>-neuron-cache-<acct>/vton-qwen/trn2) to enable.
if [ -n "${CACHE_S3:-}" ]; then
  echo "[cache] Uploading ${COMPILED_MODELS_DIR} -> ${CACHE_S3}/"
  aws s3 sync "${COMPILED_MODELS_DIR}/" "${CACHE_S3}/" --only-show-errors
