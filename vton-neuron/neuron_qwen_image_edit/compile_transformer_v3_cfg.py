...
def nki_flash_attention(query, key, value):
    ...
    need_batch_pad = vc_size == 2 and bs * n_head < 2
    if need_batch_pad:
        # Pad batch to 2 so nc(2) grid works correctly with LNC=2
        q = torch.cat([q, torch.zeros_like(q)], dim=0)
        k = torch.cat([k, torch.zeros_like(k)], dim=0)
        v = torch.cat([v, torch.zeros_like(v)], dim=0)
        attn_output = torch.zeros((2, q_len, d_head), dtype=torch.bfloat16, device=q.device)
    ...
    if need_batch_pad:
        attn_output = attn_output[:1]  # Remove padding
...
def compile_transformer_v3_cfg(args):
    ...
    with NxDParallelState(world_size=world_size, tensor_model_parallel_size=tp_degree):
        ...
        # Calculate head padding for attention weights
        ...
        # Attention weight keys that need output-dim padding (Q/K/V projections)
        ...
        # Attention weight keys that need input-dim padding (output projections)
        ...
        for key, value in model.state_dict().items():
            ...
            # Pad attention weights if needed
            if need_attn_pad:
