"""
Transformer compilation using parallel_model_trace (V1 API) with NKI Flash Attention.

Key approach:
1. Uses parallel_model_trace API (supports NKI Flash Attention)
2. RoPE frequencies computed OUTSIDE the model and passed as INPUT tensors (like V2)
3. Uses NKI Flash Attention kernel for better performance

This combines V1's NKI support with V2's RoPE handling to get the best of both.
"""
...
# Functionalization converts in-place ops to out-of-place, which breaks NKI kernels
...
from torch_neuronx.xla_impl.ops import nki_jit  # Same as Flux
...
def nki_flash_attention(query, key, value):
    """
    NKI Flash Attention wrapper for QwenImage.
...
class NKIQwenAttention(nn.Module):
    ...
    uses NeuronFluxAttention. This avoids the XLA tracing issues with diffusers'
    Attention.forward() method.

    Key design choices (matching Flux):
    1. Transpose Q, K, V to [B, H, S, D] format BEFORE attention
    2. Call NKI attention wrapper with [B, H, S, D] inputs (exactly like Flux)
    3. Transpose back after attention
    """
    ...
    def __init__(self, orig_attn):
        ...
        Args:
            orig_attn: The sharded diffusers Attention module
        """
        super().__init__()
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Forward pass with NKI Flash Attention - directly calls the kernel.
        Follows Flux's pattern: transpose to [B, H, S, D] before attention.
        """
        ...
        # Apply QK normalization (Flux does this after reshape too)
...
def replace_attention_with_nki(transformer):
    ...
    This completely replaces diffusers' Attention class with our custom
    NKIQwenAttention class, similar to how Flux uses NeuronFluxAttention.
    """
    for i, block in enumerate(transformer.transformer_blocks):
        # Replace the attention module entirely
...
class NeuronQwenTransformerV1Flash(nn.Module):
    """
    Neuron-optimized QwenImage Transformer for V1 Flash.

    Key features:
    - Uses parallel_model_trace API (supports NKI Flash Attention)
    - RoPE frequencies are passed as INPUT, not computed internally
    ...
    def __init__(self, original_transformer, tp_degree):
        ...
        # This completely replaces diffusers' Attention class with our custom class
        # that directly calls NKI kernel, similar to how Flux does it
...
class TracingWrapperV1Flash(nn.Module):
    """Wrapper for parallel_model_trace tracing."""
...
def get_transformer_model_v1_flash(tp_degree: int, img_rotary_emb: torch.Tensor, txt_rotary_emb: torch.Tensor):
    """Load and create the transformer model for parallel_model_trace."""
    ...
    neuron_transformer = NeuronQwenTransformerV1Flash(pipe.transformer, tp_degree)
    neuron_transformer = neuron_transformer.to(torch.bfloat16)
    neuron_transformer.eval()

    # Wrap for tracing
    model = TracingWrapperV1Flash(neuron_transformer)

    return model, {}


def compile_transformer_v1_flash(args):
    """Compile transformer using parallel_model_trace with NKI Flash Attention."""
    ...
    print("=" * 60)
    print("Transformer V1 Flash Compilation")
    print("=" * 60)
    ...
    # First, load model to get RoPE
    print("\nLoading model to get RoPE...")
    pipe = QwenImageEditPlusPipeline.from_pretrained(
        MODEL_ID,
        torch_dtype=torch.bfloat16,
        local_files_only=True,
    ...
    get_transformer_f = partial(get_transformer_model_v1_flash, tp_degree, img_rotary_emb, txt_rotary_emb)
    ...
    with torch.no_grad():
        ...
        print("\nTracing model with parallel_model_trace...")
        compiled_transformer = neuronx_distributed.trace.parallel_model_trace(
            get_transformer_f,
            sample_inputs,
            compiler_workdir=f"{args.compiler_workdir}/transformer_v1_flash",
            compiler_args=compiler_flags,
            tp_degree=tp_degree,
            inline_weights_to_neff=False,
            # Note: spmd_mode requires checkpoint_loader_callable, try without it first
        )

        # Save - use subdirectory for model files (parallel_model_load expects only .pt files)
        output_path = f"{args.compiled_models_dir}/transformer_v1_flash"
        model_path = f"{output_path}/model"
        os.makedirs(model_path, exist_ok=True)

        print(f"\nSaving model to {model_path}...")
        neuronx_distributed.trace.parallel_model_save(
            compiled_transformer, model_path)

        # Save config in parent directory (not with model files)
        config = {
            "height": args.height,
            "width": args.width,
        ...
        # Save pre-computed RoPE in parent directory
        torch.save({
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compiled_models_dir", type=str, default="/mnt/nvme/compiled_models")
    parser.add_argument("--compiler_workdir", type=str, default="/mnt/nvme/compiler_workdir")
    args = parser.parse_args()

    compile_transformer_v1_flash(args)
