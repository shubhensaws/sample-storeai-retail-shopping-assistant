...
def shard_qwen_attention(tp_degree: int, attn: Attention):
    ...
    # Padded total output dim (may be larger than orig if heads were padded)
    padded_out_features = total_padded_heads * dim_head  # e.g., 32*128=4096 for TP=32
    extra_heads = total_padded_heads - orig_num_heads

    def _pad_weight_out(weight, orig_features, padded_features):
        """Pad weight's output dim (dim=0) with zeros for extra heads."""
        if padded_features > orig_features:
            padding = torch.zeros(
                (padded_features - orig_features, weight.shape[1]),
                dtype=weight.dtype, device=weight.device)
            return torch.cat([weight, padding], dim=0)
        return weight

    def _pad_bias_out(bias, orig_features, padded_features):
        """Pad bias with zeros for extra heads."""
        if bias is not None and padded_features > orig_features:
            padding = torch.zeros(
                padded_features - orig_features,
                dtype=bias.dtype, device=bias.device)
            return torch.cat([bias, padding], dim=0)
        return bias

    def _pad_weight_in(weight, orig_features, padded_features):
        """Pad weight's input dim (dim=1) with zeros for extra heads."""
        if padded_features > orig_features:
            padding = torch.zeros(
                (weight.shape[0], padded_features - orig_features),
                dtype=weight.dtype, device=weight.device)
    ...
    orig_q = attn.to_q
    q_weight = _pad_weight_out(orig_q.weight.data, orig_inner_dim, padded_out_features)
    q_bias = _pad_bias_out(orig_q.bias.data if orig_q.bias is not None else None, orig_inner_dim, padded_out_features)
    ...
    attn.to_q.weight.data = get_sharded_data(q_weight, 0)
    if attn.to_q.bias is not None:
        attn.to_q.bias.data = get_sharded_data(q_bias, 0)
    del orig_q

    orig_k = attn.to_k
    k_weight = _pad_weight_out(orig_k.weight.data, orig_inner_dim, padded_out_features)
    k_bias = _pad_bias_out(orig_k.bias.data if orig_k.bias is not None else None, orig_inner_dim, padded_out_features)
    attn.to_k = ColumnParallelLinear(
        ...
        gather_output=False,
        dtype=torch.bfloat16)
    attn.to_k.weight.data = get_sharded_data(k_weight, 0)
    if attn.to_k.bias is not None:
        attn.to_k.bias.data = get_sharded_data(k_bias, 0)
    del orig_k

    orig_v = attn.to_v
    v_weight = _pad_weight_out(orig_v.weight.data, orig_inner_dim, padded_out_features)
    v_bias = _pad_bias_out(orig_v.bias.data if orig_v.bias is not None else None, orig_inner_dim, padded_out_features)
    attn.to_v = ColumnParallelLinear(
        ...
        gather_output=False,
        dtype=torch.bfloat16)
    attn.to_v.weight.data = get_sharded_data(v_weight, 0)
    if attn.to_v.bias is not None:
        attn.to_v.bias.data = get_sharded_data(v_bias, 0)
    del orig_v

    # Shard output projection (input dim needs padding to match padded QKV output)
    orig_out = attn.to_out[0]
    out_weight = _pad_weight_in(orig_out.weight.data, orig_inner_dim, padded_out_features)
    attn.to_out[0] = RowParallelLinear(
        ...
        dtype=torch.bfloat16)
    attn.to_out[0].weight.data = get_sharded_data(out_weight, 1)
    ...
    if hasattr(attn, 'add_q_proj') and attn.add_q_proj is not None:
        ...
        aq_weight = _pad_weight_out(orig_add_q.weight.data, orig_inner_dim, padded_out_features)
        aq_bias = _pad_bias_out(orig_add_q.bias.data if orig_add_q.bias is not None else None, orig_inner_dim, padded_out_features)
        attn.add_q_proj = ColumnParallelLinear(
            ...
            gather_output=False,
            dtype=torch.bfloat16)
        attn.add_q_proj.weight.data = get_sharded_data(aq_weight, 0)
        if orig_add_q.bias is not None:
            attn.add_q_proj.bias.data = get_sharded_data(aq_bias, 0)
        del orig_add_q

    if hasattr(attn, 'add_k_proj') and attn.add_k_proj is not None:
        orig_add_k = attn.add_k_proj
        ak_weight = _pad_weight_out(orig_add_k.weight.data, orig_inner_dim, padded_out_features)
        ak_bias = _pad_bias_out(orig_add_k.bias.data if orig_add_k.bias is not None else None, orig_inner_dim, padded_out_features)
        attn.add_k_proj = ColumnParallelLinear(
            ...
            gather_output=False,
            dtype=torch.bfloat16)
        attn.add_k_proj.weight.data = get_sharded_data(ak_weight, 0)
        if orig_add_k.bias is not None:
            attn.add_k_proj.bias.data = get_sharded_data(ak_bias, 0)
        del orig_add_k

    if hasattr(attn, 'add_v_proj') and attn.add_v_proj is not None:
        orig_add_v = attn.add_v_proj
        av_weight = _pad_weight_out(orig_add_v.weight.data, orig_inner_dim, padded_out_features)
        av_bias = _pad_bias_out(orig_add_v.bias.data if orig_add_v.bias is not None else None, orig_inner_dim, padded_out_features)
        attn.add_v_proj = ColumnParallelLinear(
            ...
            gather_output=False,
            dtype=torch.bfloat16)
        attn.add_v_proj.weight.data = get_sharded_data(av_weight, 0)
        if orig_add_v.bias is not None:
            attn.add_v_proj.bias.data = get_sharded_data(av_bias, 0)
        del orig_add_v

    # Shard to_add_out (input dim needs padding to match padded QKV output)
    if hasattr(attn, 'to_add_out') and attn.to_add_out is not None:
        orig_add_out = attn.to_add_out
        add_out_weight = _pad_weight_in(orig_add_out.weight.data, orig_inner_dim, padded_out_features)
        attn.to_add_out = RowParallelLinear(
            ...
            input_is_parallel=True,
            dtype=torch.bfloat16)
        attn.to_add_out.weight.data = get_sharded_data(add_out_weight, 1)
